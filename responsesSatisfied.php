...
if($count==0){
	...
	$sqlStr3 = "UPDATE im3080_responses SET satisfied = satisfied + 1 WHERE ID_num = '$ID_num'";
	$result3=mysqli_query($link, $sqlStr3);
}
//If person has responded but want to change his response from "neutral"/"unsatisfied" to "satisfied"
//Also caters to if person accidentally click "satisfied" twice
else if ($count == 1){
	$sqlStr4 = "UPDATE im3080_responses SET unsatisfied = 0 WHERE ID_num = '$ID_num'";
	...
	$sqlStr6 = "UPDATE im3080_responses SET satisfied = 0 WHERE ID_num = '$ID_num'";
	$result6=mysqli_query($link, $sqlStr6);
	$sqlStr7 = "UPDATE im3080_responses SET satisfied = satisfied + 1 WHERE ID_num = '$ID_num'";
