...
if(isset($_SESSION['student_ID'])){
	...
	$currentUser = $_SESSION['student_ID'];
}
else if(isset($_SESSION['prof_ID'])){
	//continue to load page
	$currentUser = $_SESSION['prof_ID'];
}
else{
...
<html>
	<head>
		...
		<link href = "css/course_vid.css" rel = "stylesheet">
				...
				<div class = "collapse navbar-collapse navHeaderCollapse">
					...
					<ul class = "nav navbar-nav navbar-right">
						...
						<li><a href = "#">Tools</a></li>
							...
							<a href = "#" class = "dropdown-toggle" data-toggle = "dropdown"><?php if(isset($_SESSION['student_ID'])){
																									echo 'Personal Info';  //different word for prof and student 
																								}
																								else if(isset($_SESSION['prof_ID'])){
																									echo 'Lecture Info';  //different word for prof and student
																								}?><b class = "caret"></b></a>
							
							<ul class = "dropdown-menu">
									<?php 
									for ($num = 1; $num <= $_SESSION['course_num']; $num++){
									$course_ID = $_SESSION["course_ID" . $num];
									echo "<li><a href =";
									if(isset($_SESSION['student_ID'])){
										echo "personalInfo_responses.php?course_ID=", $course_ID, ">";}   //different url for prof and student
									else if (isset($_SESSION['prof_ID'])){
										echo "lectureInfo_responses.php?course_ID=", $course_ID, ">";}    //different url for prof and student
							...
							<a href = "#" class = "dropdown-toggle" data-toggle = "dropdown"><?php echo $currentUser?><b class = "caret"></b></a>
           	...
           	<div class = "row" style="width:1370px;"> 
              	<div class = "col-md-6" style="width:670px;">
					...
					<?php 
                			 	...
                			 	$sqlCourses = "SELECT * FROM Course WHERE Course_ID ='".$_GET["course_ID" ]."'";
					...
					This browser or document mode doesn't support canvas object</canvas>
  					...
  					<video id="example_video_1" class="video-js vjs-default-skin" controls preload="metadata" width="640" height="420" 
       				poster="img/<?php echo $_GET["ID_video"];?>.png"
     				data-setup="{}">   
       
       				<source src="vid/<?php echo $_GET["ID_video"];?>.mp4" type='video/mp4' />
				    ...
				    <p class="vjs-no-js">To view this video please enable JavaScript, and consider upgrading to a web browser that <a href="http://videojs.com/html5-video-support/" target="_blank">supports HTML5 video</a></p>
				  	...
				  	<input type="checkbox" name="c1" onclick="showMe('overlay')">Off Captions 
										...
										<?php
					$course_ID_num = $_GET["ID_video"];
					echo"<span id='ID_video' style='display:none'>".$course_ID_num."</span>";
					
					$course_ID = $_GET["course_ID"];
					echo"<span id='course_ID' style='display:none'>".$course_ID."</span>";
					
					$ID_num = $_SESSION['ID_num'];
					echo"<span id='ID_num' style='display:none'>".$ID_num."</span>";
...
$ID_video = $_GET['ID_video']; //get ID_video
$ID_video_comment = $ID_video."_comment"; // created by xw; to get database table, retrieve the comments to display on screen


$mysqli->real_query('SELECT video_time, content, sending_date, sending_time, like_num, dislike_num, size, color, isAnno, position,link,ID_num FROM '.$ID_video_comment .' WHERE reply_ID = 0 ORDER BY video_time ASC')
...
<script type="text/javascript">
								function showMe(box) {
									...
									function loadOverlay (){
										...
										if(count > 0) {
											...
											if(checkBlocked(arr[count][9]) == 1) {
											count++;
											continue;
											}
											var comment = new Comment(arr[count][1],arr[count][0],arr[count][2],640,count*30+10,fontType,fontSize,arr[count][5],arr[count][8],arr[count][9]); //[0]= time  [1]= comment  [2]= anno  [5]= color
   ...
   if(arr[count][7] == 'top') { //comments marked 'top' will flow top down
		height = tHeightCounter*30+10;
		if(height < 370) { //if height has not reached the end of the canvas
			arr[count][4] = tHeightCounter*30+10; //height counter for this comment
			comment.setHeight(tHeightCounter*30+10);
		} else {
			tHeightCounter = 0;
			arr[count][4] = tHeightCounter*30+10; //once the position of the comment have reached the bottom of the canvas, position it at the top again
			comment.setHeight(tHeightCounter*30+10);
		}
		tHeightCounter++;
   } else { // comments marked 'bottom' will flow bottom up
		height = 370 - (bHeightCounter*30+10);
		if(height > 40) {
			arr[count][4] = 370 - (bHeightCounter*30+10);
			comment.setHeight(370 - (bHeightCounter*30+10));
...
<br><br>
					...
					<button type="button" onclick="loadXMLDoc1()"><img src="img/satisfied.png" border="0"width="30" height="30" class="satisfied" value="satisfied" /> : Satisfied</button>  &nbsp		 
					<script>
					...
					var url = "responsesSatisfied.php?ID_video="
				        url = url+ID_video;
					...
					<button type="button" onclick="loadXMLDoc2()"><img src="img/neutral.png" border="0"width="30" height="30" class="neutral" value="neutral"/> : Neutral</button> &nbsp
					<script>
					...
					var ID_video = "<?php echo $_GET["ID_video"]?>";  
					var url = "responsesNeutral.php?ID_video="
				        url = url+ID_video;
					xmlhttp.open("GET", url,true);		
					xmlhttp.send();
					}
					...
					<button type="button" onclick="loadXMLDoc3()"><img src="img/not-satisfied.png" border="0"width="30" height="30" class="unsatisfied" value="unsatisfied" /> :Unsatisfied</button>
					<script>
					...
					var ID_video = "<?php echo $_GET["ID_video"]?>";  
					var url = "responsesUnsatisfied.php?ID_video="
				        url = url+ID_video;
					xmlhttp.open("GET", url,true);  
					xmlhttp.send();
					}
				...
				<br><br><br>
										<br>
										<table class="table " border= "1" width = 75% align="center" style="background: #FFFFFF;"> <tr class = "info">
										<th>Hot Comments: </th>
										 <th>Video Time </th>
										  <th>Sending Date </th>
										  <th>Sending Time </th>
		 							    ...
		 							    <?php
											...
											$sqlStrLike = "SELECT *  FROM ".$_GET['ID_video']."_comment ORDER BY like_num DESC, video_time ASC LIMIT 5 ";
						...
						<div class = "col-md-6">
								...
								<div id="tabs" style = 'width:650px'>
								  <ul>
									...
									<th width=15%><span id="video_time" class = "sort">Time</span></th>
									<th width=67% >Comment</th>
									<th width=18%><span id="sending_time" class = "sort">Published on</span></th></tr>
									</table>
									<div id="commentBoxFrame"></div>
									<br>
									<!-- previous button, next page, go page, to change page -->
									<div id = "btnArea">
									<a id="previous" type="button" class="btn btn-info btn-md"><span class="glyphicon glyphicon-step-backward"></span> Previous </a> 
									
									<input id="inputTime" rows="1" cols="4">
									<a type="submit" id="go" value="Go" class="btn btn-info btn-md"><span class="glyphicon glyphicon-play" ></span> Go </a> 
									<a id="next" type="button" value="Next" class="btn btn-info btn-md"><span class="glyphicon glyphicon-step-forward"></span> Next </a> 
									...
									<!-- change:settingsBox -->
<span id = "settingsBox">
<span id = "settingsArea" style="display:table;margin-top:10px; margin-left:5px;"> 
...
<span>Choose color:</span><input class='simple_color' value='#000000' name="color" id="color" style="float:left !important">
</span></span>

									<!--text area of comments, with max capacity of 200 words-->
									&nbsp;
									<form action="" target="commentBoxIframe" name="submitComment" id="submitComment">
									<textarea id="comment" type="text" name="comment" placeholder="Maximum 2000 letters..." rows="3" cols="40" wrap=PHYSICAL></textarea>
									...
									&nbsp;
									<a type="submit" id="submit" value="Submit" class="btn btn-info btn-md"><span class="glyphicon glyphicon-collapse-up"></span> Submit </a>
									<span id="wordLeft">0</span><span>/2000</span>
									<span id="submitError" style="display:none">All fields can not be empty</span>
									
									
									<a id="settingsBtn" style="float:left" class="btn btn-info btn-md"><span class="glyphicon glyphicon-cog"></span> Settings </a> 
